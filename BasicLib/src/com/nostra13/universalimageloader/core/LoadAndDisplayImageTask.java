...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			//FIXME:
			if(bitmap == null && configuration.secDiskCache != null){
				File secImageFile = configuration.secDiskCache.get(uri);
				if (secImageFile != null && secImageFile.exists()) {
					L.d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, memoryCacheKey);
					loadedFrom = LoadedFrom.DISC_CACHE;

					checkTaskNotActual();
					bitmap = decodeImage(Scheme.FILE.wrap(secImageFile.getAbsolutePath()));
				}
			}
